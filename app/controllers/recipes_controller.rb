  ...
  def create
    ...
    if @current_user != nil 
      ...
      if the_recipe.valid?
        ...
        system_message.content =  "You are a chef. The user has #{the_recipe.ingredient} left in the fridge. Suggest a recipe that includes #{the_recipe.ingredient}. Modify the recipe based on the user's additional request. The recipe should include a section of '#Ingredients:' and a section of 'Instructions:'. At the end of the recipe, say 'Bon Appétit!' " 
      ...
      redirect_to("/", { :alert => "You must first sign up or sign in to use the generator." })
    end
  end
end
